...
export default function CenteredGrid() {
  ...
  const onSubmit = async (e) => {
    // e.preventDefault();
    await axios.put(baseURL+`sports/coach/${id}/`,
      editcoach,specialisation
    )
    .then((res) => {
  ...
  const loadUser = async (e) => {
    const result = await axios.get(baseURL+`sports/coach/${id}/`
    );
              ...
              <div className={classes.root} style={{ padding: "20px" }}>
                <form method="POST" noValidate onSubmit={formik.handleSubmit}>
                  <Grid container spacing={2}>
                    ...
                    onOpen={handleOpen}
                    value={editcoach.specialisation?.length > 1 && editcoach.specialisation || 'none'}
                    onChange={onChange}
